...
// GetBlockByNumber fetches a block by number with Avalanche transaction type support
func (c *EthereumClient) GetBlockByNumber(ctx context.Context, blockNumber *big.Int) (*types.Block, error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "transaction type not supported") ||
			...
			// Fall back to raw JSON-RPC for Avalanche transactions
...
// getBlockByNumberRaw fetches a block using raw JSON-RPC to handle Avalanche transactions
func (c *EthereumClient) getBlockByNumberRaw(ctx context.Context, blockNumber *big.Int) (*types.Block, error) {
	...
	// Parse response using raw JSON structure that matches Avalanche RPC format
...
type rawBlock struct {
	...
	BlockGasCost   string `json:"blockGasCost,omitempty"`   // hex string
	ExtDataGasUsed string `json:"extDataGasUsed,omitempty"` // hex string
...
func (c *EthereumClient) convertRawBlock(raw *rawBlock) (*types.Block, error) {
	...
	return &types.Block{
		...
		// Avalanche-specific fields
		BlockExtraData: raw.BlockExtraData,
		BlockGasCost:   raw.BlockGasCost,
		ExtDataGasUsed: raw.ExtDataGasUsed,
		ExtDataHash:    raw.ExtDataHash,
...
func (c *EthereumClient) convertGethBlock(gethBlock *ethtypes.Block) *types.Block {
	...
	return &types.Block{
		...
		Transactions:  transactions,
		// Avalanche-specific fields (empty for go-ethereum blocks, populated from raw RPC)
		BlockExtraData: "",
		BlockGasCost:   "",
		ExtDataGasUsed: "",
		ExtDataHash:    "",
...
func (c *EthereumClient) convertTransaction(tx *ethtypes.Transaction, gethBlock *ethtypes.Block, index int) (*types.Transaction, error) {
	...
	// Extract signature components with error handling for Avalanche transactions
	v, r, s := tx.RawSignatureValues()

	// Handle cases where signature components might be nil
	if v == nil {
		v = big.NewInt(0)
	}
	if r == nil {
