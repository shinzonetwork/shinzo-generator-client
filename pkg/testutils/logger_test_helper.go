...
func (tls *TestLoggerSetup) AssertLogField(fieldName, expectedValue string) {
	...
	// Check each log entry for the field. It checks both camelCase and snake_case variants, and also nested objects.
...
func (tls *TestLoggerSetup) camelToSnake(s string) string {
	...
	for i, r := range s {
		...
		result.WriteRune(unicode.ToLower(r)) // Convert to lowercase.
	}
