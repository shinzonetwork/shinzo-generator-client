...
func TestErrorLoggingPatterns(t *testing.T) {
	...
	t.Run("NetworkError with LogContext", func(_ *testing.T) {
		...
		testLogger.Logger.With("context", logCtx).Error("Failed to create block in DefraDB")
	...
	t.Run("DataError with LogContext", func(_ *testing.T) {
		testLogger.ClearBuffer()
		...
		testLogger.Logger.With("context", logCtx).Warn("Skipping transaction due to data error")
	...
	t.Run("StorageError with LogContext", func(_ *testing.T) {
		testLogger.ClearBuffer()
		...
		testLogger.Logger.With("context", logCtx).Error("Database operation failed")
	...
	t.Run("Critical SystemError with LogContext", func(_ *testing.T) {
		testLogger.ClearBuffer()
		...
		testLogger.Logger.With("context", logCtx).Error("Critical system error occurred")
...
func TestRetryLogicWithErrorLogging(t *testing.T) {
	...
	for attempt := range maxRetries {
		...
		logCtx := errors.LogContext(networkErr)
		testLogger.Logger.With("context", logCtx).Errorf("Failed to create block (attempt %d)", attempt+1)
...
func TestBlockHandlerErrorLogging(t *testing.T) {
	...
	testLogger.Logger.With("context", logCtx).Error("Failed to create block handler")
