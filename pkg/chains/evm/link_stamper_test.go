...
func TestStampBeforeWrite_Log_TxNotYetStamped(t *testing.T) {
	...
	logDocs := []map[string]any{
		{TransactionHashKeyName: "0xunknown"},
	}
...
func TestStampBeforeWrite_AccessListEntry(t *testing.T) {
	...
	aleDocs := []map[string]any{
		{AddressFieldName: "0x01"},
		{AddressFieldName: "0x02"},
	}
...
func TestRecordDocIDs_Block(t *testing.T) {
	...
	blockDocs := []map[string]any{{HashKeyName: "0xabc"}}
...
func TestRecordDocIDs_Block_NoDocIDs(t *testing.T) {
	...
	err := s.RecordDocIDs(cols.Block, []map[string]any{{HashKeyName: "0xabc"}}, nil)
...
func TestRecordDocIDs_NeverMutatesDocs(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			docs:       []map[string]any{{HashKeyName: "0xblock"}},
			ids:        []string{"block-id"},
		},
		{
			name:       "transaction",
			collection: cols.Transaction,
			docs:       []map[string]any{{HashKeyName: "0xtx1"}},
			ids:        []string{"tx-id"},
		},
		{
			name:       "log",
			collection: cols.Log,
			docs:       []map[string]any{{TransactionHashKeyName: "0xtx1"}},
			ids:        []string{"log-id"},
		},
		{
			name:       "accessListEntry",
			collection: cols.AccessListEntry,
			docs:       []map[string]any{{AddressFieldName: "0x01"}},
			ids:        []string{"ale-id"},
		},
	}
...
func TestUniformProtocol_FullBlockSequence(t *testing.T) {
	...
	groups := []chains.DocumentGroup{
		{Collection: cols.Block, Docs: []map[string]any{{HashKeyName: "0xblock"}}},
		{Collection: cols.Transaction, Docs: []map[string]any{{HashKeyName: "0xtx1"}}},
		{Collection: cols.Log, Docs: []map[string]any{{TransactionHashKeyName: "0xtx1"}}},
...
func TestStampBeforeWrite_Transaction_InvalidDocFails(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			txDocs := []map[string]any{{HashKeyName: tc.txHash}}
			err := s.StampBeforeWrite(cols.Transaction, txDocs)
...
func TestRecordDocIDs_Transaction_InvalidHashFails(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Contains(t, err.Error(), HashKeyName)
			_, hasEmpty := s.txHashToID[""]
			assert.False(t, hasEmpty, "empty hash must never be registered")
...
func TestStampBeforeWrite_Log_InvalidDocFails(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			logDocs := []map[string]any{{}}
			if tc.logHash != nil {
				logDocs[0][TransactionHashKeyName] = tc.logHash
			}

			err := s.StampBeforeWrite(cols.Log, logDocs)

			require.Error(t, err)
			assert.Contains(t, err.Error(), TransactionHashKeyName)
...
func TestLog_CorruptionRegression(t *testing.T) {
	...
	s.blockID = "block-doc-id-1"
	txDocs := []map[string]any{{HashKeyName: "0xtx1"}}
