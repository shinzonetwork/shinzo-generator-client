...
func TestStampBeforeWrite_Log(t *testing.T) {
	...
	logDocs := []map[string]any{
		{TransactionHashFieldName: "0xtx1"},
	}
...
func TestStampBeforeWrite_Log_TxNotYetStamped(t *testing.T) {
	...
	logDocs := []map[string]any{
		{TransactionHashFieldName: "0xunknown"},
	}
...
func TestRecordDocIDs_NeverMutatesDocs(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			name:       "log",
			collection: cols.Log,
			docs:       []map[string]any{{TransactionHashFieldName: "0xtx1"}},
			ids:        []string{"log-id"},
		},
		{
...
func TestStampBeforeWrite_Log_InvalidDocFails(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			logDocs := []map[string]any{{}}
			if tc.logHash != nil {
				logDocs[0][TransactionHashFieldName] = tc.logHash
			}

			err := s.StampBeforeWrite(cols.Log, logDocs)

			require.Error(t, err)
			assert.Contains(t, err.Error(), TransactionHashFieldName)
